...
$('body').append('<div class="modal fade" id="session-timeout-dialog"> \
    <div class="modal-dialog"> \
      <div class="modal-content"> \
        ...
        <div class="modal-footer">' + showButtonsHtml + '</div> \
      </div> \
    </div> \
   </div>');
...
if (!opt.ignoreUserActivity) {
  ...
  startSessionTimer();
    ...
    if ($('#session-timeout-dialog').length > 0 &&
      ...
      $(document).prop('title', 'Active');
      }
    });
  }

// Keeps the server side connection live, by pinging url set in keepAliveUrl option.
// KeepAlivePinged is a helper var to ensure the functionality of the 
