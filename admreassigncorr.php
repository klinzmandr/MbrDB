...
if ($action == '') {
	print <<<pagePart1
<h3>Reassignment of Correspondence Record MCIDs</h3>
<p>This utility will replace the MCID field of specified correspondence record(s) with a new one.  </p>
<p>Corresondence records to be reassigned are identified by individual record numbers entered into the input field seperated by commas.  </p>
<p>The new MCID is specified in the input field and must be one of an existing member record.  </p>
<p>The correspondence records identified will not be changed other than to have the new MCID replace the current one.  </p>

<a class="btn btn-warning" href="admreassigncorr.php?action=form">CONTINUE</a><br /><br />
...
if ($action == 'form') {
	...
	print <<<inputForm
<form class="form" action="admreassigncorr.php">
...
<a class="btn btn-primary" href="admreassigncorr.php">RETURN</a>
</form>	
<script src="jquery.js"></script><script src="js/bootstrap.min.js"></script></body></html>
...
if ($action == 'apply') {
	...
	if ($rowcount == 0) {
		...
		<a class="btn btn-warning" href="admreassigncorr.php?action=form">RESTART</a>';
		exit;
		}
	...
	foreach ($rnarray as $m) {
		$where = "`CORID` = '$m'";
		sqlupdate("correspondence",$flds,$where);
		//echo "where: $where<br />";
		echo "Correspondence record $m updated, new MCID is $mcid<br />";
		}
